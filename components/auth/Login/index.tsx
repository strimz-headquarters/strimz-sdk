...
const FormInputs = () => {
    ...
    //initial form values
    const initialValues: LoginFormInputValues = {
        email: "",
        password: ""
    };
    ...
    const onSubmit = async (
        values: LoginFormInputValues,
        { resetForm }: FormikHelpers<LoginFormInputValues>
    ) => {
        try {
            setIsSending(true);
            ...
            {(formik) => {
                ...
                return (
                    <Form className="w-full flex flex-col gap-3 mt-6">
                        ...
                        {/* btn */}
                        <button type="submit" disabled={!(dirty && isValid)} className='w-full h-[40px] flex justify-center items-center rounded-[8px] bg-accent text-[#FFFFFF] font-poppins font-[600] shadow-joinWaitlistBtnShadow text-shadow text-[14px] disabled:opacity-80 disabled:cursor-not-allowed'>
                            {
                                isSending ?
                        ...
                        <div className="w-full flex flex-col items-center gap-4 mt-8">
                            <p className="font-poppins text-center font-[400] text-[14px] text-[#58556A] leading-[24px]">Don&apos;t have an account? <Link href="/signup" className="font-poppins font-[600] text-[14px] text-accent hover:underline leading-[24px]">Sign Up</Link>
                            </p>
